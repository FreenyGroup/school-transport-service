import SocalIcons from '@/components/ui/socalIcons';
import React from 'react';
import {
  FaPhone,
  FaEnvelope,
  FaLocationDot,
  FaArrowRight,
} from 'react-icons/fa6';

const ExtraInfo = () => {
  return (
    <div className="mt-5">
      <div className="mt-5 mb-6">
        <a
          href=""
          className="p-3 bg-primary text-cream-foreground rounded-md flex items-center gap-2.5  btn after:bg-green"
        >
          {' '}
          Get In Touch{' '}
          <span>
            <i className="flex items-center">
              <FaArrowRight className="text-cream-foreground" />{' '}
            </i>
          </span>{' '}
        </a>
      </div>
      <div>
        <h4 className="text-xl font-bold text-[#385469]">
          Contact Info
        </h4>
        <ul className="mt-5 flex flex-col gap-[15px]">
          <li className="flex items-center gap-2">
            <FaPhone className="text-primary-foreground" />{' '}
            <a href="tel:571453185" className="ml-2.5">
              (571) 453-1850
            </a>
          </li>
          <li className="flex items-center gap-2">
            <FaPhone className="text-primary-foreground" />{' '}
            <a href="tel:5712450806" className="ml-2.5">
              (571) 245-0806
            </a>
          </li>
          <li className="flex items-center gap-2">
            <FaEnvelope className="text-primary-foreground" />{' '}
            <a
              href="mailto:info@schooltransportservices.com"
              className="ml-2.5"
            >
              info@schooltransportservices.com
            </a>
          </li>
          <li className="flex items-center gap-2">
            <FaLocationDot className="text-primary-foreground" />{' '}
            <span className="ml-2.5">
              308 Hillwood Ave., Falls Church, VA 22046
            </span>
          </li>
        </ul>
      </div>
      {/*
      <SocalIcons
        className={'w-11 h-11 bg-white text-muted-foreground'}
      />
      */}
    </div>
  );
};

export default ExtraInfo;
